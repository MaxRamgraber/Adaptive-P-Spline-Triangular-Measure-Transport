...
if "models" not in os.listdir(root_directory):
    os.mkdir(root_directory+"\\"+"models")
...
for random_seed in np.arange(0,101,1):

    print("random seed: "+str(random_seed).zfill(4))
    ...
    while counter < 10:
        ...
        if counter == 0:
            np.random.seed(random_seed + counter)
        else:
            np.random.seed(random_seed + int(N*counter))
        
        counter += 1
        
        try:
            
            if "results_"+str(random_seed).zfill(4)+".p" not in list(os.listdir(root_directory)):
                ...
                srf.transform("normal_to_uquad")  # also "arcsin" works
                
                log_hk = copy.copy(srf.field)
                log_hk -= np.min(log_hk)
                log_hk /= np.max(log_hk)
                log_hk = hk_min + (hk_max-hk_min)*log_hk
                ...
                pickle.dump(results,open(root_directory+"\\"+"models"+"\\"+"results_"+str(random_seed).zfill(4)+".p","wb"))
                ...
                plt.savefig(root_directory+"\\"+"models"+"\\"+"img_"+str(random_seed).zfill(4)+".png",bbox_inches="tight")
                plt.close("all")
