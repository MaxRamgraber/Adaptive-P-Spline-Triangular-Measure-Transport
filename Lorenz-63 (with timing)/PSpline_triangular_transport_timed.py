...
class adaptive_spline_transport():
    ...
    def train_map(self, X, sparsity = None, lambda_initial = 2.0, optimize_lambdas = True, beta_initial = None):
        ...
        # These timings isolate optimization/adaptation from map evaluation.
        # - outer_optimization_seconds: smoothing-parameter optimization
        # - inner_optimization_seconds: coefficient optimization at fixed lambda
        # - optimization_seconds: outer + inner
        # Diagnostics and basis/setup costs are stored separately but not included
        # in optimization_seconds.
        time_train_map_begin = time.perf_counter()
        self.latest_timing = {
            "train_map_seconds"             : np.nan,
            "outer_optimization_seconds"    : 0.0,
            "inner_optimization_seconds"    : 0.0,
        ...
        for d in np.arange(self.skip_dimensions,self.D,1):
            ...
            time_component_begin = time.perf_counter()
            ...
            if optimize_lambdas: # Yes, we do solve the outer optimization problem
                ...
                time_outer_begin = time.perf_counter()
                ...
                outer_optimization_seconds = time.perf_counter() - time_outer_begin
            ...
            time_inner_begin = time.perf_counter()
            ...
            inner_optimization_seconds = time.perf_counter() - time_inner_begin
            
            # Append the optimal betas
            self.betas[d] = copy.copy(beta)
            
            time_diagnostics_begin = time.perf_counter()
            ...
            self.nlls[d] = self.nll_jit(
                ...
                X_basis     = [X_basis_local[key] for key in list(X_basis_local.keys())], 
                dBxj        = dBxj)
            diagnostics_seconds = time.perf_counter() - time_diagnostics_begin
            
            component_timing = {
                "component"                    : int(d),
                "outer_optimization_seconds"   : float(outer_optimization_seconds),
                "inner_optimization_seconds"   : float(inner_optimization_seconds),
                "optimization_seconds"         : float(outer_optimization_seconds + inner_optimization_seconds),
                "diagnostics_seconds"          : float(diagnostics_seconds),
                "component_total_seconds"      : float(time.perf_counter() - time_component_begin)
            }
            
            self.latest_timing["component_timings"][int(d)] = component_timing
            self.latest_timing["outer_optimization_seconds"] += float(outer_optimization_seconds)
            self.latest_timing["inner_optimization_seconds"] += float(inner_optimization_seconds)
            self.latest_timing["optimization_seconds"] += float(outer_optimization_seconds + inner_optimization_seconds)
            self.latest_timing["diagnostics_seconds"] += float(diagnostics_seconds)
            
        self.latest_timing["train_map_seconds"] = float(time.perf_counter() - time_train_map_begin)
